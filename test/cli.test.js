...
describe('Studio API (HTTP)', () => {
  ...
  function cleanTestScenarios(dir) {
    if (!existsSync(dir)) return
    readdirSync(dir)
      .filter(f => f.startsWith('test_autogenerated'))
      .forEach(f => rmSync(join(dir, f)))
  }

  before(async () => {
    // Remove any leftover files from a previous run before starting
    cleanTestScenarios(join(FIXTURE, 'scenarios'))
    server = spawn(process.execPath, [FLUX, 'run', FIXTURE, `--port=${PORT}`], {
      cwd: ROOT,
      stdio: ['ignore', 'pipe', 'pipe'],
    })
